...
class Command(BaseCommand):
    ...
    def _compute_window(self, account):
        """
        Start at the OLDEST last_order among NON-broken machines.
        If none found (all broken or no machines), use now-7d.
        End is 'now' (rounded up to minute).
        """
        tz_now = timezone.now()
        ...
        # end_dt is exclusive; add a small delta to include the current minute
    ...
    def _upsert_row(self, row, acc):
        ...
        return obj, created
    def _run_cycle(self, page_size, log):
        ...
        for acc in accounts:
            ...
            # 2) compute window from oldest NON-broken last_order -> now
            start_dt, end_dt = self._compute_window(acc)
            ...
            for chunk_start, chunk_end in _seven_day_chunks(start_dt, end_dt):
                ...
                while True:
                    ...
                    if page * page_size >= total:
                        break
                    page += 1
                    time.sleep(15) # Sleep for 10 seconds between pages to avoid rate limiting
